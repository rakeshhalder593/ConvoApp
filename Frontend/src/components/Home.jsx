...
const [downloadError,setDownloadError]=useState("")
  ...
  const handleSubmit=async(event)=>{
    ...
    try {
      ...
      setDownloadError("")
      setConvert("File Converted Sucessfully")
    } catch (error)  {
      console.log(error);
      if (error.response && error.response.status==400) {
        setDownloadError("Error ocurring",error.response.data.message);
      }else{
        setConvert("");
      }
    }
    ...
    <div className='max-w-screen-2xl mx-auto container px-6 py-3 md:px-40'>
      <div className='flex h-screen items-center justify-center'>
        <div className='border-2 border-dashed px-4 py-2 md:px-8 md:py-6 border-indigo-400 rounded-lg shadow-lg'>
          <h1 className='text-3xl font-bold text-center mb-4'>Convert Word to PDF format Online</h1>
          <p className='text-sm text-center mb-5'>
            ...
            without having to install any software
              ...
              <div className='flex flex-col items-center space-y-4'>
                ...
                className='text-white bg-blue-500 hover:bg-blue-700 disabled:bg-gray-400 disabled:pointer-events-none duration-300 font-bold py-2 px-4 rounded-lg'>Convert File
                </button>
                {convert && (
